...
const AnimatedAbout = () => {
    ...
    const about = {
        ...
        brand: "Desde 2016, em Belém, a ACOMPORTAMENTAL oferece qualificação profissional a estudantes e profissionais da Psicologia e áreas afins. Com base na ciência comportamental, promovemos desenvolvimento pessoal e profissional, proporcionando um espaço acolhedor para aprendizado, crescimento e transformação.",
    };

    return (
        <div className='flex w-full gap-5 md:justify-between'>
            <div className='flex items-center justify-center w-1/2 gap-2'>
                {['nicole', 'fabiane'].map((item, index) => (
                    <motion.div
                        key={index}
                        onClick={() => handleClick(item)}
                        initial={{ width: "6.5rem" }}
                        animate={{ width: visibleItem === item ? "16rem" : "6.5rem" }}
                        ...
                        ${visibleItem === item ? `items-end bg-[url('/image-${item}.jpg')]` : `md:bg-left md:bg-[url('/image-${item}.jpg')]`}`}
                    >
                        ...
                        ${visibleItem !== item ? "-rotate-90 md:rotate-0 md:self-end md:py-7 md:bg-[#BF7269] md:p-0 tracking-wider h-0 md:text-lg" : ""}`}>
